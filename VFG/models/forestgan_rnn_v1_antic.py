...
class ForestGANRNN_v1_antic(BaseModel):
    def __init__(self, opt):
        
        super(ForestGANRNN_v1_antic, self).__init__(opt)
        self._name = 'forestgan_rnn_v1_antic'
        self._opt = opt
        self._T = opt.T
        self._extra_ch_Gf = 2
        self._extra_ch_Gb = 0
    ...
    def _create_generator_f(self):
        return NetworksFactory.get_by_name('generator_wasserstein_gan_f_static_ACR_v1_antic', c_dim=self._extra_ch_Gf, T=self._opt.T,use_moment=self._use_moments)
    ...
    def _generate_fake_samples_test(self, t, use_moments):
        if use_moments:
            Inext_fake_fg, mask_next_fg, features = self._Gf(self._curr_f, self._curr_OFs)
            Inext_fake_bg = self._Gb(self._curr_b)
            Inext_fake = (1 - mask_next_fg) * Inext_fake_bg + Inext_fake_fg
            return Inext_fake, Inext_fake_fg, Inext_fake_bg, mask_next_fg, features
        else:
            Inext_fake_fg, mask_next_fg = self._Gf(self._curr_f, self._curr_OFs)
            Inext_fake_bg = self._Gb(self._curr_b)
            Inext_fake = (1 - mask_next_fg) * Inext_fake_bg + Inext_fake_fg
    ...
    def forward(self, T):
        ...
        with torch.no_grad():
            for t in range(T-1):
                self._move_inputs_to_gpu(t)
                if self._use_moments:
                    Inext_fake, Inext_fake_fg, Inext_fake_bg, mask_next_fg, feature = self._generate_fake_samples_test(t, self._use_moments)
                    features.append(feature)
                else:
                    Inext_fake, Inext_fake_fg, Inext_fake_bg, mask_next_fg = self._generate_fake_samples_test(t, self._use_moments)
                self._curr_f = Inext_fake_fg 
                self._curr_b = Inext_fake_bg
        ...
        if self._use_moments:
            return fgs, bgs, fakes, masks, features
        else:
