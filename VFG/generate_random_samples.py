...
class Train:
    def __init__(self):
        ...
        self._dataset_val = DavisDataset(self._opt, self._opt.T_val, self._opt.val_dir)
        
        self._data_loader_val = data.DataLoader(self._dataset_val, self._opt.val_batch_size, drop_last=True, shuffle=True)


        self._dataset_val_size = len(self._dataset_val)
        print('#val videos = %d' % self._dataset_val_size)
    ...
    def _generate_something(self):
        ...
        for i_val_batch, val_batch in enumerate(self._data_loader_val):
            self._model.set_input(val_batch)
            fgs, bgs = self._model.forward(self._opt.T_val)

        def rgb2bgr(img):
            return cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        for t in range(self._opt.T_val-1):
            cv2.imwrite(os.path.join(self._opt.val_dir_save,'fg_'+ "{:02d}".format(t) + '.jpeg'), rgb2bgr(tensor2im(fgs[t])))
            cv2.imwrite(os.path.join(self._opt.val_dir_save,'bg_'+ "{:02d}".format(t) + '.jpeg'), rgb2bgr(tensor2im(bgs[t])))
