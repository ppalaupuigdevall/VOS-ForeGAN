...
class BaseOptions():
    ...
    def initialize(self):
        ...
        self._parser.add_argument('--gpu_ids',          type=str,   default='2',                                    help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')

        self._parser.add_argument('--model',            type=str,   default='forestgan_rnn_v5',                   help='there is only this one so do not change it')
        self._parser.add_argument('--load_epoch',       type=int,   default=-1,                                 help='which epoch to load? set to -1 to use latest cached model')
        self._parser.add_argument('--name',             type=str,   default='exp_forestgan_rnn_v52',                        help='name of the experiment. It decides where to store samples and models')
